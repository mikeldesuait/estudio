...
async function mostrarAreas(main) {
    ...
    for (let area of areas) {
        ...
        if (area.id === 'herramientas' || area.id === 'networking' || area.id === 'pnl' || area.id === 'dieta') {
            const color = area.id === 'herramientas' ? '#8b5cf6' : area.id === 'networking' ? '#00b4d8' : area.id === 'dieta' ? '#e67e22' : '#9b59b6';
...
async function mostrarAsignaturas(main, areaId, cursoId, semestreId) {
    const area = areas.find(a => a.id === areaId);
    if (!area) return;
    const esH = (areaId === 'herramientas' || areaId === 'networking' || areaId === 'pnl' || areaId === 'dieta');
